...
HaxballJS.then(HBInit => CreateRoom(HBInit, roomConfigs[0]));

function CreateRoom(HBInit, config) {
  const room = HBInit({
...
room.onPlayerJoin = async function (player) {
  ...
  if (giriscikisWebhook != "") {
    var giriscikisfields = [
      ...
      {
        name: "```👤 Oyuncu Sayısı```",
        value: `\`\`\`c\n(${playersAll.length + 1}/${room.maxPlayers})\`\`\``,
        inline: false,
      }
    ]
    var giriscikisembed = {
    ...
    if (!kickFetchVariable) {
      if (giriscikisWebhook != "") {
        var giriscikisfields = [
          ...
          {
            name: "```👤 Oyuncu Sayısı```",
            value: `\`\`\`c\n(${playersAll.length}/${room.maxPlayers})\`\`\``,
            inline: false,
          }
        ]
        var giriscikisembed = {
